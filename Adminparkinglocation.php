...
class ParkingLocationManager {
    ...
    public function renderSearchForm() {
        echo '<form name="search" action="Adminparkinglocation.php" method="POST">
    ...
    public function renderParkingLocationsTable($qRes) {
        ...
        if ($qRes->num_rows > 0) {
            while (($Row = $qRes->fetch_row())) {
                    ...
                    echo "<td>{$Row[4]}</td>";
                    echo "<td>{$Row[5]}</td>";
                    echo "<td class='btn'><a href='Adminparkinginform.php?id={$Row[0]}&location={$Row[1]}' class='btn btn-primary'>View</a></td></tr>\n";
            }
        } else {
...
try {
    ...
    // Call the method with the type parameter
